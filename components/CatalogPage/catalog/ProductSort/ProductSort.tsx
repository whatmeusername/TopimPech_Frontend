...
function ProductSort({ disabled }: { disabled?: boolean }): JSX.Element {
	...
	const searchParams =
		typeof window !== 'undefined' ? new URLSearchParams(window.location.search) : ({} as URLSearchParams);

	const modal = useRef<HTMLDivElement>(null!);
	const [modalActive, setModalActive] = useModal(modal);

	const selected = selectedSort
		? sortOptions.find((s) => s.slug === selectedSort) ?? (sortOptions[0] as SortItem)
		: (sortOptions[0] as SortItem);
